...
void main() {
  group('OtaProgressOverlay', () {
    ...
    testWidgets('shows "Downloading update" + 0% for downloading with no fraction',
        (tester) async {
      final state = ValueNotifier(const OtaOverlayState(
    ...
    testWidgets('skips version line when current equals target', (tester) async {
      final state = ValueNotifier(const OtaOverlayState(
        phase: PatchApplyPhase.downloading,
    ...
    testWidgets('error variant uses default hint when none given', (tester) async {
      final state = ValueNotifier(const OtaOverlayState(
        hasError: true,
        errorText: 'io error',
      ));
  ...
  group('OtaOverlayManager', () {
    ...
    test('OtaOverlayState.copyWith can clear hasError by setting other fields',
        () {
