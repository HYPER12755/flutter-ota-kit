...
class _HangingDatabase implements DatabasePlugin {
  ...
  Future<Paginated<List<Bundle>>> getBundles(
          DatabaseBundleQueryOptions options) async =>
      Paginated(
        data: const <Bundle>[],
...
void main() {
  group('FlutterPatcher.configure*', () {
    test('configureSupabase stores the config (no thrown error)', () {
      FlutterPatcher.configureSupabase(const SupabaseUpdateConfig(
    ...
    test('configurePostgres stores the config', () {
      FlutterPatcher.configurePostgres(const PostgresUpdateConfig(
        host: 'localhost',
        database: 'ota',
        channel: 'production',
        platform: Platform.android,
    ...
    test('configureCloudflare stores the config', () {
      FlutterPatcher.configureCloudflare(const CloudflareUpdateConfig(
        databaseId: 'db',
        accountId: 'acct',
    ...
    test('configureAws stores the config', () {
      FlutterPatcher.configureAws(const AwsUpdateConfig(
        bucketName: 'b',
        region: 'us-east-1',
        accessKeyId: 'k',
        secretAccessKey: 's',
    ...
    test('configurePocketBase stores the config', () {
      FlutterPatcher.configurePocketBase(const PocketBaseUpdateConfig(
  ...
  group('performSharedUpdateCheck timeout', () {
    test('throws TimeoutException when the DB call exceeds the timeout',
        () async {
