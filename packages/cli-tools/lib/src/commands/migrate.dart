...
class MigrateCommand extends FlutterPatcherCommand {
  ...
  /// Postgres, supabase = PostgREST, aws/standalone = no relational SQL).
  ...
  Future<int> run() => runGuarded(() async {
        ...
        // self-hosted server). Short-circuit before any SQL discovery so a
        // missing or empty (README-only) migrations dir does not abort.
        if (provider == 'aws' || provider == 'standalone') {
          ...
          box('migrate', [
            ...
            '  • standalone  → self-hosted bundle store, no relational SQL',
            '',
        ...
        if (provider == 'supabase') {
          ...
          throw StateError(
            ...
            '`flutter-patcher init`) — a Supabase Management API key, OR\n'
