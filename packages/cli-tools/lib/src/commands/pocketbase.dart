...
class PocketBaseInstallCommand extends FlutterPatcherCommand {
  ...
  @override
  ArgParser get argParser => ArgParser()
    ..addOption('version', help: 'PocketBase version to install.');
  ...
  Future<int> run() => runGuarded(() async {
        ...
        if (result.alreadyInstalled) {
          ...
          step('Downloaded ${(result.bytesDownloaded / 1024 / 1024).toStringAsFixed(1)} MB');
...
class PocketBaseServeCommand extends FlutterPatcherCommand {
  ...
  Future<int> run() => runGuarded(() async {
        ...
        final adminEmail = r['admin-email'] as String? ??
            Platform.environment['POCKETBASE_ADMIN_EMAIL'];
        final adminPassword = r['admin-password'] as String? ??
        ...
        step('Starting PocketBase on http://$host:$port (data: ${dataDir.path})...');
        await manager.start(
...
class PocketBaseStopCommand extends FlutterPatcherCommand {
  ...
  @override
  String get description => 'Hint to stop a running PocketBase (Ctrl+C in serve).';
...
class PocketBaseStatusCommand extends FlutterPatcherCommand {
  ...
  Future<int> run() => runGuarded(() async {
        ...
        if (exists) {
          step('PocketBase v${paths.version}: installed at ${paths.binaryPath.path}');
        } else {
          step('PocketBase: not installed. Run `flutter_ota_kit pocketbase install`.');
        }
        return;
      });
}
