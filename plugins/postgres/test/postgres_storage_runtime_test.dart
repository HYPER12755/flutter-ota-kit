...
void main() {
  group('Postgres storage runtime', () {
    test(
      'getDownloadUrl throws a clear error when no servingBaseUrl is set',
      () async {
    ...
    test(
      'getDownloadUrl builds an http url when servingBaseUrl is set',
      () async {
        final storage = postgresStorage(
          PostgresStorageConfig(
            db: PostgresConfig(
        ...
        final url = (await storage.profiles.runtime!.getDownloadUrl(
          'postgres://bundles/abc/zip',
        ))['fileUrl'];
