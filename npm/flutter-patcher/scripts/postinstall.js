...
// `flutter-patcher-<os>-<arch>` binary ships in `bin/`, use it. Otherwise, when
...
if (dartCheck.status !== 0) {
  console.warn(
    `flutter-patcher: Dart SDK not found; skipping build. Provide a prebuilt ` +
      `binary (bin/flutter-patcher-${platformName}-${archName}${ext}) or install ` +
...
if (!fs.existsSync(path.join(cliDir, 'pubspec.yaml'))) {
  console.warn('flutter-patcher: bundled Dart source not found; skipping build.');
  process.exit(0);
}
